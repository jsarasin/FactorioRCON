...
class MainWindow:
    ...
    def event_button_connect_clicked(self, button):
        ...
        self.pyrcon = PyRCON("ws://" + server['address'] + ':' + server['port'] + '/' + server['password'])
    ...
    def add_console_message_to_buffer(self, message, mtime):
        ...
        if mtime is None:
            ...
            timestring = datetime.fromtimestamp(mtime).isoformat()

        console_message = console_message + timestring + " " + message.replace("\\n", "\n").replace("\\r", '').strip()

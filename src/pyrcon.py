...
class PyRCON(WebSocketClient):
    ...
    def closed(self, code, reason=None):
        if PyRCON.DEBUG:
            print("PyRCON closed(reason=" + reason +")")
    ...
    def received_message(self, message):
        if PyRCON.DEBUG:
            print("PyRCON received_message(message=" + message + ")")
    ...
    def send_chat_message(self, message, text_size=14, font_color="#FFFFFF", italic=False):
        if PyRCON.DEBUG:
            print("PyRCON send_chat_message(message=%s, text_size=%i, font_color=%s, italic=%s)" %(message, text_size, font_color, str(italic)))
    ...
    def send_console_callback(self, cmd, callback):
        if PyRCON.DEBUG:
            print("PyRCON send_console_callback(cmd=" + cmd + "callback=" + str(callback))
